      ...
      <div class="row" style="width: 100%;">
        ...
        <?php 
          if($userID == 'unknown'){
            ...
            $query = "SELECT * FROM purchaseditem where UserID = '$userID' && StatusID <= 3";
                ...
                <div class="float-right" style="background-image: url(&quot;assets/img/icons8-more-48.png&quot;);width: 24px;height: 24px;background-size: contain;margin-top: 5%;margin-right: 8%;">
                  ...
                  <div class="collapse" id="collapse-<?php echo $i ?>" style="background-color: #f8f8f8;width: 150px;">
                    ...
                    <?php 
                      ...
                      $result_userID = mysqli_query($con, $query);
                      $row_toUserID = mysqli_fetch_array($result_userID);

                       ?>
                      <button class='btn text-left' type='button' onclick="reportVendor(<?php echo $row_toUserID['UserID'];?> )">Report Vendor</button>
                      <?php 
                      if($itemStaus == 1)
                      {echo "<button class='btn text-left' type='button' onclick='" . 'changeItemStatus(5,' .$row['OrderID']. ")'>Hide</button>";}
                      else if($itemStaus == 2)
                      {echo "<button class='btn text-left' type='button' onclick='" . 'changeItemStatus(5,' .$row['OrderID']. ")'>Hide</button>";}
                      else if($itemStaus == 3)
                      {echo "<button class='btn text-left' type='button' onclick='" . 'changeItemStatus(6,' .$row['OrderID']. ")'>Cancel</button>";}
                      ?>
